...
if ($token) {
    // Check if token exists - using customer_email column to store admin_email
    $stmt = $db->prepare('SELECT customer_email, expires_at FROM password_resets WHERE token = ?');
    $stmt->execute([$token]);
    $resetData = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($resetData) {
        $admin_email = $resetData['customer_email']; // This contains admin_email for admin resets
        $expires_at = $resetData['expires_at'];
        
        // Verify it's actually an admin email (check if exists in Staff table with admin role)
        $stmt_check = $db->prepare('SELECT staff_email FROM Staff WHERE staff_email = ? AND role = "admin"');
        $stmt_check->execute([$admin_email]);
        $admin = $stmt_check->fetch(PDO::FETCH_ASSOC);
        
        if ($admin && $expires_at && strtotime($expires_at) > time()) {
            $show_form = true;
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (!$hasLength || !$hasUpper || !$hasLower || !$hasNumber || !$hasSpecial) {
                    ...
                    $stmt2->execute([$hash, $admin_email]);
            ...
            if (!$admin) {
                $message = 'Invalid token for admin account.';
            } else {
...
<body class="admin-page">
    ...
    <div class="auth-container">
        ...
        <div class="auth-main">
            <form class="auth-form admin-theme" action="reset-password.php?token=<?php echo htmlspecialchars($token); ?>" method="post">
                <div class="form-header">
                    <h1 style="color: #A26E60;">Reset Password</h1>
                    <p>Admin Account</p>
                </div>
                ...
                <?php if ($show_form): ?>
                    ...
                    <button type="submit" class="submit-btn" style="background: #A26E60;">Reset Password</button>
                <?php else: ?>
                    ...
                    <div class="switch-form" style="margin-top: 20px;">
                        <a href="forgot-password.php" style="color: #A26E60;">Request New Reset Link</a>
                    </div>
                <?php endif; ?>
