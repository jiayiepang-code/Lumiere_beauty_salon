...
$(document).ready(function () {
  ...
  fetch("http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3", {
    ...
    body: JSON.stringify({
      location: "js/booking.js:28",
      message: "Booking.js DOM ready",
      data: {
        step1Exists: !!document.getElementById("step-1"),
        headerExists: !!document.querySelector(".main-header"),
        pageWrapperExists: !!document.querySelector(".page-wrapper"),
        staffDataAvailable: !!window.staffData,
      },
      timestamp: Date.now(),
  ...
  $(".service-card").click(function (e) {
    ...
    fetch("http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "booking.js:93",
        message: "Service selection clicked",
        data: {
          serviceId: serviceId,
          serviceIdType: typeof serviceId,
          dataAttr: $(this).data("service-id"),
    ...
    if ($(this).hasClass("selected")) {
      ...
      fetch(
        "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "booking.js:105",
            message: "Service added to selection",
            data: {
              serviceId: serviceId,
              selectedServicesLength: selectedServices.length,
              lastAdded: selectedServices[selectedServices.length - 1],
              allSelected: selectedServices,
            },
            timestamp: Date.now(),
            sessionId: "debug-session",
            runId: "run1",
...
function removeService(serviceId) {
  ...
  $(".service-card").each(function () {
    ...
    if (select.length > 0 && select.val() == serviceId)
...
function updateSummary() {
  ...
  if (selectedServices.length === 0) {
    container.html('<p class="text-muted">Select services to see summary</p>');
    return;
  }
  ...
  // Date Header
  if (selectedDate) {
    const dStr = new Date(selectedDate).toLocaleDateString("en-US", {
      weekday: "long",
      day: "numeric",
      month: "long",
    });
    ...
    if (selectedTime) {
      ...
      selectedServices.forEach((id) => {
        if (sData[id]) totDur += parseInt(sData[id].duration);
      });
      ...
      html += `<div class="text-muted small mt-1">Select a time</div>`;
    }
    html += `</div>`;
  }
  ...
  selectedServices.forEach((sid, idx) => {
    ...
    if (stid != 0 && stid != "0" && window.staffData) {
      ...
      if (found) {
        const displayName =
          found.first_name + (found.last_name ? " " + found.last_name : "");
    ...
    if (
      s.sub_category &&
      s.sub_category.trim() !== "" &&
      s.sub_category.toLowerCase() !== "general"
    ) {
      ...
      fetch(
        "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "booking.js:279",
            message: "Service display formatted with sub_category",
            data: {
              serviceId: sid,
              serviceName: s.name,
      ...
      fetch(
        "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "booking.js:285",
            message: "Service display using service name only",
            data: {
              serviceId: sid,
              serviceName: s.name,
              subCategory: s.sub_category || "none",
              displayName: serviceDisplayName,
            },
            timestamp: Date.now(),
            sessionId: "debug-session",
                    ...
                    <div class="row g-0">
                        <div class="col-8">
                            <div class="d-flex align-items-center mb-1">
                                <span class="fw-bold text-dark me-2"><span style="font-family: 'Segoe UI', sans-serif;">${
                                  idx + 1
                        ...
                        <div class="col-4 text-end">
                            <span class="fw-bold text-dark" style="font-family: 'Segoe UI', sans-serif;">RM ${parseFloat(
                              s.price
                            ).toFixed(2)}</span>
                        </div>
                    </div>
  ...
  html += `
        <div class="mt-auto">
            <div class="d-flex justify-content-between border-top pt-3"><span class="h5">Total</span><span class="h5" style="color: var(--accent-gold);">RM ${totalP.toFixed(
              2
            )}</span></div>
        </div>`;
...
function showDateLimitMessage() {
  ...
  const message = $(
    '<div id="date-limit-message" class="alert alert-info mt-3 mb-0" style="display:block; animation: fadeIn 0.3s;">' +
...
function loadTimeSlots() {
  ...
  container.html(
    '<div class="text-center py-3"><div class="spinner-border" role="status" style="color: #c29076;"></div></div>'
  );
  let totalDuration = 0;
  const serviceData = window.allServicesData || {};
  selectedServices.forEach((id) => {
    if (serviceData[id]) totalDuration += parseInt(serviceData[id].duration);
  });
  $("#total-duration-display").text(totalDuration);

  setTimeout(() => {
    container.empty();
    $("#estimated-time-container").remove();
    let hasDisabled = false;

    // Use server time instead of client time to ensure consistency
    // Fallback to client time if server time is not available
    const serverNow = window.serverTime
      ? new Date(window.serverTime)
      : new Date();
    ...
    for (let h = SHOP_OPEN_HOUR; h < SHOP_CLOSE_HOUR; h++) {
      for (let m = 0; m < 60; m += 30) {
        ...
        // Create slot date using selected date and time
        const slotDate = new Date(selectedDate + "T" + timeStr);
        const selectedDateOnly = new Date(selectedDate);
        selectedDateOnly.setHours(0, 0, 0, 0);

        // Check if slot is in the past based on server time
        // Only disable if it's today and the slot time has passed according to server time
        let isPast =
        ...
        container.append(
          `<div class="${classes}" data-time="${timeStr}" data-minutes="${startMins}">${timeStr}</div>`
        );
      }
    }
    ...
    $(".time-slot:not(.disabled)").click(function () {
      $(".time-slot").removeClass("selected duration-occupied");
      $(this).addClass("selected");
      selectedTime = $(this).attr("data-time");
      const start = parseInt($(this).attr("data-minutes"));
      const end = start + totalDuration;
      $(".time-slot").each(function () {
        const curr = parseInt($(this).attr("data-minutes"));
        if (curr > start && curr < end) $(this).addClass("duration-occupied");
      });
      updateEstimatedTimeDisplay(selectedTime, totalDuration);
      updateSummary();
    });
  }, 300);
}
...
function updateEstimatedTimeDisplay(start, duration) {
  ...
  $("#estimated-time-container").html(
    `<div class="estimated-time-message"><i class="fas fa-clock me-2"></i><strong>${start} - ${end}</strong><br><small class="text-muted">Total: ${duration} mins</small></div>`
  );
}

// STAFF SELECTION
function loadStaffSelection() {
  const container = $("#staff-selection");
  container.empty();
  const sData = window.allServicesData || {};
  ...
  selectedServices.forEach((sid) => {
    ...
    let sName = "No Preference";
    let btnText = "Select Staff";
    let style = "border: 2px solid var(--secondary-beige);";
    if (curStaff != 0 && curStaff != "0") {
      btnText = "Change";
      style =
        "border: 2px solid var(--accent-gold); background: rgba(212, 175, 55, 0.05);";
  ...
  $(document)
    .off("click", ".staff-selection-card")
    .on("click", ".staff-selection-card", function (e) {
      e.stopPropagation();
...
function openStaffModal(sid) {
  ...
  const suggestedStaffEmails =
    staffServiceMap[serviceIdStr] || staffServiceMap[sid] || [];

  // Add "No Preference" option
  const isNo = selectedStaff[sid] == 0;
  list.append(
    `<div class="d-flex justify-content-between p-3 border rounded mb-2 bg-light staff-option" data-service-id="${sid}" data-staff-id="0" style="cursor: pointer; ${
      isNo ? "border-color: var(--accent-gold) !important;" : ""
    }"><span class="fw-bold">No Preference</span><button class="btn btn-sm ${
      isNo ? "btn-warning" : "btn-outline-secondary"
    } rounded-pill staff-select-btn" data-service-id="${sid}" data-staff-id="0" onclick="event.stopPropagation(); selectStaff('${sid}', 0);">${
      isNo ? "Selected" : "Select"
    }</button></div>`
  );
  ...
  if (staffData) {
    ...
    allStaffMembers.forEach(({ member, dbRole }) => {
      ...
      const hasService =
        suggestedStaffEmails.includes(member.staff_id) ||
        suggestedStaffEmails.includes(member.staff_email) ||
        suggestedStaffEmails.some(
          (email) =>
            String(email).toLowerCase() ===
              String(member.staff_id).toLowerCase() ||
            String(email).toLowerCase() ===
              String(member.staff_email).toLowerCase()
        );
      ...
      // Method 3: Service name keyword matching (fallback - match any role that has relevant keywords)
        ...
        if (
          serviceText.includes("facial") ||
          serviceText.includes("anti-aging") ||
          serviceText.includes("cleansing")
        ) {
          if (dbRoleLower.includes("beaut") || dbRoleLower.includes("facial")) {
            shouldInclude = true;
          }
        } else if (
          serviceText.includes("massage") ||
          serviceText.includes("hot stone") ||
          serviceText.includes("aromatherapy")
        ) {
          if (dbRoleLower.includes("massage")) {
            shouldInclude = true;
          }
        } else if (
          serviceText.includes("hair") ||
          serviceText.includes("cut") ||
          serviceText.includes("styling")
        ) {
          if (dbRoleLower.includes("hair") || dbRoleLower.includes("stylist")) {
            shouldInclude = true;
          }
        } else if (
          serviceText.includes("nail") ||
          serviceText.includes("manicure") ||
          serviceText.includes("pedicure") ||
          serviceText.includes("gel")
        ) {
          if (
            dbRoleLower.includes("nail") ||
            dbRoleLower.includes("manicure")
          ) {
            shouldInclude = true;
          }
        }
      ...
      if (shouldInclude) {
        ...
        if (suggestedStaffEmails.length > 0) {
          ...
          fetch(
            "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
            {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                location: "booking.js:598",
                message: "Using staffServiceMap (service_id match)",
                data: {
                  staffName: member.first_name,
                  staffId: member.staff_id,
                  staffEmail: member.staff_email,
                  serviceId: sid,
                  isSuggested: isSuggested,
                  suggestedStaffEmails: suggestedStaffEmails,
                },
                timestamp: Date.now(),
                sessionId: "debug-session",
                runId: "post-fix",
        ...
        if (!isSuggested) {
          ...
          if (staffPrimaryServices && service.name) {
            ...
            const primaryServicesLower = staffPrimaryServices
              .toLowerCase()
              .trim();

            // Split primary_services by common separators (&, comma, etc.) and check each part
            const serviceParts = primaryServicesLower
              .split(/[&,]/)
              .map((s) => s.trim())
              .filter((s) => s.length > 0);
            ...
            isSuggested = serviceParts.some((part) => {
              ...
              if (partTrimmed === serviceNameNormalized) {
                ...
                fetch(
                  "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
                  {
                    ...
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      location: "booking.js:625",
                      message: "Fallback: Exact match found",
                      data: {
                        part: partTrimmed,
              ...
              if (index !== -1) {
                ...
                const afterChar =
                  afterIndex < partTrimmed.length
                    ? partTrimmed[afterIndex]
                    : " ";

                // Only match if surrounded by word boundaries (space, dash, or start/end)
                if (
                  (beforeChar === " " || beforeChar === "-" || index === 0) &&
                  (afterChar === " " ||
                    afterChar === "-" ||
                    afterChar === "," ||
                    afterIndex === partTrimmed.length)
                  ...
                  fetch(
                    "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
                    {
                      ...
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
                        location: "booking.js:642",
                        message: "Fallback: Phrase match with word boundaries",
                        data: {
                          part: partTrimmed,
                          serviceNameNormalized: serviceNameNormalized,
                          staffName: member.first_name,
            ...
            fetch(
              "http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3",
              {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  location: "booking.js:658",
                  message: "Fallback string matching result",
                  data: {
                    isSuggested: isSuggested,
                    staffName: member.first_name,
    ...
    list.append(`
            <div class="d-flex justify-content-between p-3 border rounded mb-2 bg-white staff-option" 
                ...
                <div class="flex-grow-1">
                    ...
                    <small class="text-muted d-block mt-1">${dbRole
                      .replace("_", " ")
                      .replace(/\b\w/g, (l) => l.toUpperCase())}</small>
                </div>
                <div class="d-flex align-items-center">
                    <button class="btn btn-sm ${
                      isSel ? "btn-warning" : "btn-outline-secondary"
                    } rounded-pill staff-select-btn" 
                            data-service-id="${sid}" 
                            data-staff-id="${staffIdEscaped}"
  ...
  $(document)
    .off("click", ".staff-option")
    .on("click", ".staff-option", function (e) {
  ...
  $(document)
    .off("click", ".staff-select-btn")
    .on("click", ".staff-select-btn", function (e) {
      e.stopPropagation();
      e.preventDefault();
      const serviceId = $(this).data("service-id");
      const staffId = $(this).data("staff-id");
...
function updateStaffHeader() {
  const disp = $("#staff-display-name");
  const icon = $("#staff-icon-container");
  const uStaff = [];
  ...
  if (uStaff.length === 0) {
    disp.text("No Preference / Any Staff");
    icon.html('<i class="fas fa-users text-muted"></i>');
  } else if (uStaff.length === 1) {
    disp.text(uStaff[0]);
    icon.html('<i class="fas fa-user" style="color: #c29076;"></i>');
  } else {
    disp.text(`Multiple Staff (${uStaff.join(", ")})`);
    icon.html(
      '<div class="d-flex"><i class="fas fa-user" style="color: #c29076;"></i><i class="fas fa-user text-success ms-n1"></i></div>'
    );
  }
}
...
function validateCurrentStep() {
  ...
  if (currentStep === 4 && !$("#payment-method").val()) {
...
function updateStepIndicator() {
  ...
  for (let i = 1; i <= 4; i++) {
    if (i < currentStep) $(`#step-${i}-indicator`).addClass("completed");
    else if (i === currentStep) $(`#step-${i}-indicator`).addClass("active");
  }
}
function updateNavigationButtons() {
  $("#prev-btn").toggle(currentStep > 1);
  $("#next-btn").html(currentStep === 4 ? "Confirm" : "Next");
}
function submitBooking() {
  ...
  fetch("http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3", {
    ...
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      location: "booking.js:841",
      message: "Before normalization",
      data: {
        selectedServices: selectedServices,
        selectedServicesType: typeof selectedServices[0],
        selectedServicesLength: selectedServices.length,
      },
      timestamp: Date.now(),
      sessionId: "debug-session",
  ...
  const normalizedServices = selectedServices.filter(
  ...
  fetch("http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3", {
    ...
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      location: "booking.js:857",
      message: "After normalization",
      data: {
        normalizedServices: normalizedServices,
        normalizedLength: normalizedServices.length,
      },
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run2",
  ...
  Object.keys(selectedStaff).forEach((serviceId) => {
    ...
    normalizedStaff[serviceId] =
      staffValue === 0 || staffValue == "0" ? 0 : staffValue;
  });
  ...
  fetch("http://127.0.0.1:7242/ingest/03464b7d-2340-40f5-be08-e3068c396ba3", {
    ...
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      location: "booking.js:862",
      message: "Final data being sent",
      data: {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res.success) {
        ...
        console.error(
          "Booking failed. Full server response:",
